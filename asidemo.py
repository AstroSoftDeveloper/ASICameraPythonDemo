...
import cv2
import numpy as np
asi = CDLL("libASICamera2.dylib") # 调用SDK lib
...
def init(): # 相机初始化
    ...
    startx = 0
    starty = 0
    imageType = ASI_IMG_TYPE.ASI_IMG_RAW8

    err = asi.ASISetROIFormat(cameraID, width, height, _bin, imageType.value) # SetROIFormat
    if err != 0:
...
def getFrame(): # 获取图像帧
    ...
    getDataAfterExp.argtypes = [c_int, POINTER(c_ubyte), c_int]
...
def setExpValue(val): # 设置曝光时间
    ...
    err = asi.ASISetControlValue(cameraID, ASI_CONTROL_TYPE.ASI_EXPOSURE.value, val, ASI_BOOL.ASI_FALSE.value)

def setGainValue(val): # 设置增益
    err = asi.ASISetControlValue(cameraID, ASI_CONTROL_TYPE.ASI_GAIN.value, val, ASI_BOOL.ASI_FALSE.value)